...
- (void)createViewWithOptions:(NSDictionary *)options {
    ...
    [self setButtonCallback:@"buttonCallback"];
...
- (void)destroyMap:(CDVInvokedUrlCommand *)command {
    ...
    if (self.imageButton) {
        ...
        [self.imageButton removeTarget:self
...
- (void)addMapPins:(CDVInvokedUrlCommand *)command {
    ...
    for (int y = 0; y < pins.count; y++) {
        ...
        CDVAnnotation *annotation = [[CDVAnnotation alloc] initWithCoordinate:pinCoord
                                                                        index:index
                                                                        title:title
                                                                     subTitle:subTitle
                                                                     imageURL:imageURL];
...
- (MKAnnotationView *)mapView:(MKMapView *)theMapView
    ...
    NSString *identifier = [NSString stringWithFormat:@"INDEX[%i]",
                                                      phAnnotation.index];
    ...
    if (self.buttonCallback && phAnnotation.index != -1) {
        ...
        [rightButton setTag:phAnnotation.index];
    ...
    if (phAnnotation.selected) {
        [self performSelector:@selector(openAnnotation:)
                   withObject:phAnnotation
                   afterDelay:1.0];
    }
...
//    NSString *jsString = [NSString stringWithFormat:@"%@(\"%i\");",
//                                                    self.buttonCallback,
//                                                    tmpButton.tag];
//    [self.webView stringByEvaluatingJavaScriptFromString:jsString];

//    NSString *annotationTapFunctionString = [NSString stringWithFormat:@"%s%@%s", "annotationTap('", [view.annotation title], "')"];
    int tagValue = tmpButton.tag;
    NSString *eval = [NSString stringWithFormat:@"mapKit.onMapCallback(%d);", tagValue];
